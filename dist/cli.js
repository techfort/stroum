...
function compileCommand(args) {
    ...
    try {
        ...
        const resolver = new module_resolver_1.ModuleResolver(stdlibPath);
        ...
        const validator = new validator_1.Validator(stdlibPath);
        let allIssues = [];
        ...
        const transpiler = new transpiler_1.Transpiler(stdlibPath);
        ...
        transpiler_1.Transpiler.emitRuntime(outputDir);
...
function runCommand(args) {
    ...
    // Execute stroum-run
    const child = (0, child_process_1.spawn)(scriptPath, [inputFile], {
        stdio: 'inherit',
        shell: true
    });
    child.on('exit', (code) => {
