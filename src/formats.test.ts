...
const fixturesPath = (filename: string) =>
...
describe("Formats Module", () => {
  describe("read_csv", () => {
    ...
    it("should handle non-existent CSV file", async () => {
      await expect(read_csv(fixturesPath("nonexistent.csv"))).rejects.toThrow();
    });
  ...
  describe("read_json", () => {
    ...
    it("should handle non-existent JSON file", async () => {
      await expect(
        read_json(fixturesPath("nonexistent.json")),
      ).rejects.toThrow();
    });
    ...
    it("should handle invalid JSON", async () => {
      ...
      await expect(read_json(invalidJsonPath)).rejects.toThrow();
  ...
  describe("read_parquet", () => {
    ...
    it("should handle non-existent Parquet file", async () => {
      await expect(
        read_parquet(fixturesPath("nonexistent.parquet")),
      ).rejects.toThrow();
    });
    ...
    it("should handle invalid Parquet file", async () => {
      ...
      await expect(read_parquet(invalidPath)).rejects.toThrow();
  ...
  describe("read_avro", () => {
    ...
    it("should handle non-existent Avro file", async () => {
      await expect(
        read_avro(fixturesPath("nonexistent.avro")),
      ).rejects.toThrow();
    }, 10000);
    ...
    it("should handle invalid Avro file", async () => {
      ...
      await expect(read_avro(invalidPath)).rejects.toThrow();
  ...
  describe("infer_schema", () => {
    ...
    it("should handle non-existent file", async () => {
      await expect(
        infer_schema(fixturesPath("nonexistent.csv")),
      ).rejects.toThrow();
    });
  });
  ...
  describe("Format Comparison", () => {
    it("should produce equivalent data across all formats", async () => {
      ...
      for (let i = 0; i < parquetData.length; i++) {
        ...
        expect(Math.abs(parquetData[i].score - avroData[i].score)).toBeLessThan(
          0.01,
        );
      }
    });
