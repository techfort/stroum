...
export function analyzeDataflow(module: AST.Module): DataflowGraph {
  ...
  function scanNodes(
    expr: AST.Expression | AST.IndentedBody | null | undefined,
  ): void {
    if (!expr) return;
    ...
    switch (expr.type) {
      ...
      case "ParallelExpression": {
        ...
        if (expr.gatherPipe.target.type === "CallExpression") {
          walkArgs(
            (expr.gatherPipe.target as AST.CallExpression).args,
            gatherTarget,
          );
        }
        if (expr.gatherPipe.streamEmit) {
  ...
  function walkArgs(args: AST.Expression[], sourceId: string | null): void {
    for (const arg of args) {
      if (
        arg.type === "Identifier" &&
        nodes.has(`fn:${arg.name}`) &&
        !nodes.get(`fn:${arg.name}`)?.isExternal
      ) {
  ...
  function walkExprChildren(
    expr: AST.Expression | null | undefined,
    sourceId: string | null,
  ): void {
    if (!expr) return;
    switch (expr.type) {
  ...
  for (const expr of module.primaryExpressions) {
    if (expr.type === "PipeExpression" && expr.stages.length > 0) {
      ...
      if (firstId) {
        walkPipe(
          { ...expr, stages: expr.stages.slice(1) } as AST.PipeExpression,
          firstId,
        );
      } else {
        walkPipe(expr, null);
